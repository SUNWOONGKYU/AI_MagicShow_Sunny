---
id: 17
title: "Claude Code 오케스트레이터 + 멀티 CLI (Codex/Gemini/Grok/Perplexity)"
type: B
group_id: 3
group_name: "실행 방법"
order_in_group: 2
...
# 17. Claude Code 오케스트레이터 + 멀티 CLI (Codex / Gemini / Grok / Perplexity)

## 한 줄 정의

Claude Code 를 **지휘자(오케스트레이터)** 로 두고 Codex CLI · Gemini CLI · Grok CLI · Perplexity 를 각자 **특화된 실행 도구(용병)** 로 호출 — *대화형 협업*에서 *실행형 협업*(실행 → 로그 → 검증 → 재실행 루프)으로 격상시키는 멀티 모델 운용 패턴. MagicShow 강의자료 **슬라이드 26번**(매직 8 위치, 분량이 아니라 n번째 슬라이드 — [분류표 SSOT](../_PHASE/2026_05_06__19.50_분류표_확정.md#사실-키-ssot-single-source-of-truth)) *"매직 8: Claude Code로 다른 AI 용병 부리기"* 의 본체.
...
또 하나 중요한 부수 효과 — **각 CLI 가 독립 컨텍스트** 라는 점. Gemini CLI 에 200MB 짜리 PDF 를 던져도 Claude Code 메인 세션의 컨텍스트는 *"Gemini 에 던졌다"* 한 줄만 차지한다. 메인 컨텍스트는 지휘만 담당하니 끝까지 깨끗하다. 이건 #30(컨텍스트 관리 3종) 의 가장 강력한 도구이기도 하다.
...
`deploy-subagent-기본` 8대 철칙 중 5번 — *"용병 투입: 서브 에이전트로 안 되면 외부 AI(Codex, Gemini, Grok, Perplexity) 활용"* — 의 강점 분리.

| CLI | 핵심 강점 | 본인 활용 패턴 | 모델 |
|-----|----------|----------------|------|
| **Codex CLI** | 리팩터링·코드 생성 / 이미지 생성 (GPT Image) | TS·Python 함수 단위 리팩터링, GPT Image 1 호출 | GPT-5 / GPT Image 1 |
<!-- Codex CLI를 *Claude 측 커스텀 스킬*로 감싸 호출하는 시도(=Codex CLI 스킬 호출)는 본인 사용 결과 효용 낮음으로 제외했다. 본 항목의 "Codex CLI"는 그 시도가 아니라 Codex 자체를 직접 Bash로 호출하는 형태를 가리킨다. -->
...
| **Perplexity** | 출처 첨부 검색 질의 | *"근거 있는 한 줄 답"* 필요 시 | Sonar |
...
SKILL_ATLAS 사고 교훈 — *"curl 200 ≠ 동작함"*. 외부 CLI 산출물 검증도 데이터 레이어 KPI(파일 크기 / 빌드 통과 / 테스트 개수) + 사용자 여정 KPI(브라우저 클릭 / 실제 화면 확인) 둘 다 측정해야 통과. 단일 KPI 만 보고 *Verified* 처리 금지.
...
5개 컴퓨터 × 6개 Claude Code = 30 인스턴스 운용 시(#5), 각 인스턴스가 개별로 Codex/Gemini/Grok CLI 를 호출하면 API 키 충돌·rate limit 폭주가 발생한다. 본인은 *Gemini 29키 로테이션* (`ai-image-기본` 내장) 처럼 키 풀을 분배하고, Grok 처럼 단일 키 모델은 *리서치 전담 인스턴스 1개* 만 호출 권한 부여. #32(API 비용 최적화) 와 짝.

비용 측면 — Perplexity 는 단일 질의 토큰 효율이 가장 좋아 *"한 줄 답"* 용도로 빈번 호출, Gemini 3 Pro 는 대용량이라 1회 호출 비용 높지만 컨텍스트 절약 효과가 그 이상. 본인 월간 API 사용 비용의 약 60% 가 Gemini 대용량 처리, 25% Codex 코드 생성, 10% Perplexity, 5% Grok. *(측정: 본인 월간 각 CLI 청구서 합산 비율, 30개 운용 환경 기준)*

## 관련 항목

- **#5 5컴퓨터 30개 운용** — 멀티 CLI 가 가로 확장의 동력
- **#9 군대 소대 편제** — 멀티 CLI = 외부 용병 4명, 본인 분대장과 분리
- **#26 1개 AI Multi Role** — 단일 인스턴스 다역할 vs 17번 다인스턴스 다역할
- **#27 Subagent vs Agent Teams** — 내부 위임 메커니즘, 17번은 외부 용병
- **#30 컨텍스트 관리** — 외부 CLI 가 메인 컨텍스트 보호 도구
- **#32 API 비용 최적화** — 멀티 CLI 비용 통제 사전 설계
- **#40 MCP 서버** — 17번이 CLI 레벨, MCP 는 데이터·시스템 레벨 협업
- **#47 멀티 모델 이미지 제작** — 17번을 이미지 트랙에 적용한 정점 사례
