...
sources:
  ...
  - "Claude-Wiki/333-knowledge-share/wiki/2026_05_03_21.00_import_SKILL_v3_pre_refactor.md (외부 스킬 다중 버전 누적 — v1·v2·v2_260306·v3_pre_refactor 본인 톤 재단 흔적)"
---
...
| 분류 | 대표 스킬 | 비고 |
|------|----------|------|
| 본인 발명·자가 코어 | `청룡-sal-grid-dev` · `백호-platoon-formation` · `주작-sal-da` · `현무-buzzlab-simulation` · `mbo-코어5` · `review-evaluate-코어1` · `pro-persona-debate-코어6` · `5times-debug-loop-코어3` | 외부 발굴 不可, 본인이 4,000시간으로 만든 자산 |
...
| 인프라·배포 | `vercel-private-url-배포` · `cpc-setup` · `cpc-engage` · `cpc-add-project` | 본인 30개 운용 인프라 |
| 도메인 비즈 | `buzzlab-politician-winning-strategy` · `campaign-route-strategy` · `weekly-opinion-report` | 정치 분석 도메인 자산 |

발굴 → 자산화 카테고리 8개가 결정적이다. 4,000시간 가운데 가장 큰 ROI는 *"내가 못 만들 스킬을 잘 발굴해 본인 운용에 흡수한 부분"* 이고, 이 카테고리가 그 증거다.
...
- **#17 오케스트레이터·멀티 CLI** — 발굴된 스킬을 다중 CLI(Codex/Gemini/Grok)와 결합해 운용
- **#22 지능형 챗봇 만들기** — 발굴 능력이 챗봇 자산화의 전제
- **#15 SVG → 스킬 자산화** — 본인이 만드는 자산화 흐름의 짝
- **#23 CLAUDE.md 활용** — 자산화 후 헌법 등재 단계
- **#27 Subagent vs Agent Teams** — 발굴된 에이전트의 편성 단계
- **#9 군대 소대 편제 + 백호 스킬** — 발굴 후 편성 단계
- **#6 4종 품질관리** — 발굴 검증 단계의 5기준 평가
- **#35 SAL Grid 개발방법론** — 발굴 자체를 Task로 박는 골격
