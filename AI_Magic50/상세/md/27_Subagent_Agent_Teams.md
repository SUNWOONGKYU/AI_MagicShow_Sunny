...
+ 외부 용병 4명 (Codex / Gemini / Grok / Perplexity — #17)
...
| 다관점 페르소나 토론 (#13) | 부족 | **O** (각 페르소나가 팀메이트) |
| 단순 조사 1건 | O (haiku) | 과잉 |
| 풀스택 24시간 작업 | 부족 | **O** |
| Stage 간 인계 보장 | 약함 | **O** |
...
Subagent 든 팀메이트든 **자기 산출물을 자기가 검증하면 안 된다** (SKILL_ATLAS 교훈). Bravo 가 만든 UI 는 Charlie 가 Playwright 로 클릭 검증, Alpha 가 만든 API 는 Bravo 가 호출 검증. *Task Agent ≠ Verification Agent* 원칙은 Agent Teams 에서 더 엄격하게 강제된다 (백호 스킬에 박혀 있음).
...
장기 SAL Grid 프로젝트(특허 10-2026-0009425 — *#35*)에서는 Stage 1~5 를 분대장 5명에게 분배하고 인계는 직통 메시지로 처리. 메인은 Stage Gate 통과 보고만 받음. 4,000시간 누적에서 가장 큰 컨텍스트 효율 향상이 이 변경에서 나왔다 — *메인 컨텍스트가 8시간 작업 후에도 30% 미만* 유지.
...
- **#5 5컴퓨터 30개 운용** — Agent Teams 가 30개 분산의 골격
- **#9 군대 소대 편제** — 백호 스킬과 직접 짝
- **#13 전문가 페르소나 토론** — 페르소나 = 팀메이트
- **#17 오케스트레이터 + 멀티 CLI** — 17번이 외부 용병, 27번이 내부 팀
- **#23 CLAUDE.md 활용** — 환경변수·스킬 규칙을 글로벌 CLAUDE.md 에 박기
- **#26 1개 AI Multi Role** — 단일 인스턴스 다역할 vs 27번 다인스턴스
- **#30 컨텍스트 관리** — Agent Teams 가 메인 컨텍스트 보호 도구
- **#46 스크린샷 자율 검증** — Verification Agent 자리에 들어가는 도구
