...
공유할 결과물을 **단일 HTML**로 모아 `vercel --prod --yes --name <영문>` 한 줄로 띄워, **URL 아는 사람만** 접근 가능하고 검색엔진에는 노출되지 않게 즉시 배포하는 본인 표준 공유 채널. (주의: `--name` 옵션은 Vercel CLI에서 공식 deprecated — 신규 프로젝트는 `vercel link`로 프로젝트명을 미리 연결하는 방식이 권장되나, 기존 배포 흐름에서는 여전히 동작함)
