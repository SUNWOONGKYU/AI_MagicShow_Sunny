---
id: 38
title: "나에게 필요한 Skill, Agent 등의 도구를 쉽게 찾거나 직접 제작해서 활용하기"
subtitle: "필요한 도구는 찾거나 만든다 — 외부 생태계 발굴이 1순위, 없거나 안 맞으면 직접 제작"
type: A
group_id: 5
group_name: "환경"
order_in_group: 8
...
sources:
  ...
  - "~/.claude/skills/skill-create/SKILL.md (Claude Code 스킬 제조 공장)"
  - "~/.claude/skills/llm-dependent-agent-create/SKILL.md (LLM 의존형 에이전트 제조 공장)"
  - "AI 에이전트 이해 및 제작방법 가이드 — https://ai-agent-guide-finder-world.vercel.app/ (참고자료)"
---

# 38. 나에게 필요한 Skill, Agent 등의 도구를 쉽게 찾거나 직접 제작해서 활용하기

## 한 줄 정의

필요한 스킬·에이전트는 둘 중 하나로 손에 넣는다 — **찾거나, 만든다**. 1순위는 외부 오픈 생태계(skills.sh · revfactory/skills · GitHub 공개 저장소)에서 **검색 → 검증 → 자기 자산화**하는 3단계 발굴이고, 발굴해도 없거나 본인 운용에 맞지 않으면 `skill-create` · `llm-dependent-agent-create` 같은 제작 스킬로 **직접 만든다**. 본인 글로벌 환경 `~/.claude/skills/` 41개 스킬은 이 *발굴 + 제작* 두 경로가 누적된 결과다 — 절반 이상이 발굴 루프로 들어왔고, 외부에 없던 본인 고유 방법론 도구는 제작 루프로 채워졌다.
...
네 번째 이유 — **발굴에는 천장이 있다**. 외부 생태계가 아무리 넓어도 두 가지 경우엔 찾기가 막힌다. 하나, 애초에 그런 도구가 세상에 없을 때. 둘, 비슷한 도구는 있지만 본인 고유 방법론 — SAL Grid 개발방법론·MBO 목표관리·소대 편제 운용 — 전용으로 재단된 것이 없을 때. 본인 코어 스킬 `청룡-sal-grid-dev` · `백호-platoon-formation` · `mbo-코어5` 가 그 증거다. 이런 도구는 GitHub 어디를 뒤져도 안 나온다. 본인이 만든 방법론이기 때문이다. 그래서 이 노하우는 *찾기* 하나로 완결되지 않는다. **찾기가 막히는 지점에서 제작이 시작된다** — 그리고 제작 또한 `skill-create` · `llm-dependent-agent-create` 라는 *제작 전용 스킬*로 절차화되어 있다. 도구를 손에 넣는 길은 둘 — 찾거나, 만든다.
...
### 제작 — 찾기가 막혔을 때의 두 번째 경로

발굴 3단계 루프의 ① 검색 단계 *"후보 없음 → 직접 작성 검토"* 칸, 그것이 제작 경로의 진입점이다. 발굴이 막히면 만든다. 본인 글로벌에는 제작 전용 스킬 두 개가 있다.

| 제작 스킬 | 만드는 대상 | 출하 판정 |
|-----------|-------------|-----------|
| `skill-create` | Claude Code 스킬 (SKILL.md) | 단일 책임 · 자기완결 · 500줄 이내 |
| `llm-dependent-agent-create` | LLM 의존형 에이전트 | 구동 방식 8종(사람 개입형 5 / 무인 자율형 3) 중 자동 판별 |

핵심은 — **제작조차 발굴에서 출발한다**. 두 스킬 모두 *"막연한 요청을 스무고개로 충분히 기획 → 공개 저장소·OSS·Hugging Face에서 기존 자산을 발굴·해부·검증 → 건강한 부품만 이식 → QC 출하검사"* 라는 같은 철학을 공유한다. 맨땅에서 코드를 짜는 게 아니라, 검증된 부품을 조립한다. 즉 본 챕터의 두 경로 — 찾기와 제작 — 는 분리된 게 아니라 *발굴*이라는 같은 뿌리에서 갈라진 가지다. 찾기는 *완성품*을 발굴하고, 제작은 *부품*을 발굴해 조립한다.

### 찾기 vs 제작 — 판단 기준

좌표화(요구사항 한 줄 정의) 직후, 찾기로 갈지 제작으로 갈지 가른다.

| 상황 | 경로 |
|------|------|
| 동일 기능의 외부 스킬이 검증 KPI를 통과 | **찾기** — 발굴 → 자산화 |
| 외부에 있으나 본인 운용(한국어·보안 가드·SAL Grid·MBO)과 충돌 | **제작** 또는 fork 후 재단 |
| 외부에 아예 없음 / 본인 고유 방법론 전용 도구 | **제작** |

원칙은 단순하다 — *찾기 우선, 제작 차선*. 검증된 완성품이 이미 있는데 새로 만드는 것은 시간 낭비다. 그러나 찾기가 막힌 지점에서 *"없으니 포기"* 가 아니라 *"없으니 만든다"* 로 넘어가는 것 — 그 전환이 이 노하우의 절반이다.
...
3단계(SKILL.md Read 1차 필터)에서 후보가 전멸했거나, 4단계 트라이얼에서 본인 운용 충돌이 반복되면 — 거기서 멈추지 않고 제작으로 전환한다.

- **스킬이 필요하면** `skill-create` 호출 → 스무고개 기획 → 공개 저장소·OSS에서 기존 절차 발굴·해부·안전성 진단 → 건강한 부품만 이식 → QC 출하검사한 SKILL.md 완성.
- **LLM 의존형 에이전트가 필요하면** `llm-dependent-agent-create` 호출 → 동일 흐름 + 구동 방식 8종 자동 판별. (Claude Code 자체 기능에 종속된 서브에이전트 편성은 #16·#14의 영역이고, 여기서 만드는 것은 *LLM 두뇌에 의존하는 독립 에이전트*다.)

제작 산출물도 발굴 산출물과 똑같이 검증 KPI 5지표 · Verification Agent 별도 투입 · *"curl 200 ≠ 동작함"* 원칙을 통과해야 글로벌 자산화된다. **제작이라고 자기 검증이 면제되지 않는다** — 오히려 본인이 만든 것일수록 자기 검증의 사각지대가 크다. 만든 주체와 검증 주체를 분리하는 원칙은 제작 경로에서 더 엄격히 적용한다.
...
또 하나 — 발굴은 본인이 *"필요해진 시점"* 이 아니라 *"필요할 가능성이 보이는 시점"* 에 한다. 30개 동시 운용에서 그날 그날 필요한 스킬을 그날 발굴하면 이미 늦다. 본인은 매주 1회 `npx skills check`와 함께 *"이번 주 작업 시계열에서 사용할 가능성 30% 이상의 스킬"* 을 미리 발굴한다. 이 선제 발굴 루틴이 31번 30개 운용 · 14번 소대 편제 · 15번 멀티 CLI를 끊김 없이 돌리는 숨은 골격이다.

> Vault 사례: `333-knowledge-share/wiki/2026_05_03_21.00_import_SKILL_v3_pre_refactor.md` 같은 SKILL_v1 / v2 / v2_260306 / v3_pre_refactor 다중 스냅샷이 한 폴더에 누적돼 있다는 사실 자체가 *발굴 → 검증 → 본인 톤으로 재단 → 자산화 → 정기 점검*의 5단 흐름을 본인이 실제로 돌렸다는 흔적이다. 동일 스킬이 여러 버전으로 박혀 있고 *pre_refactor* 라벨이 붙은 것은 — 외부에서 들여온 원본을 본인 운용 톤(소대 편제 / Subagent 트리거 / Hooks 가드)에 맞춰 재단했다는 의미. 38번 본 항목의 *"발굴은 시작이고, 자산화는 운용 끝까지 따라간다"* 명제가 그 누적 스냅샷 자체로 증명된다.

## 관련 항목

- **#15 오케스트레이터·멀티 CLI** — 발굴된 스킬을 다중 CLI(Codex/Gemini/Grok)와 결합해 운용
- **#24 지능형 챗봇 만들기** — 발굴 능력이 챗봇 자산화의 전제
- **#22 SVG → 스킬 자산화** — 본인이 만드는 자산화 흐름의 짝
- **#33 CLAUDE.md 활용** — 자산화 후 헌법 등재 단계
- **#16 Subagent vs Agent Teams** — 발굴된 에이전트의 편성 단계
- **#14 군대 소대 편제 + 백호 스킬** — 발굴 후 편성 단계
- **#28 4종 품질관리** — 발굴 검증 단계의 5기준 평가
- **#13 SAL Grid 개발방법론** — 발굴 자체를 Task로 박는 골격
- **find-skills-기본 (본인 글로벌)** — 본 노하우의 *찾기* 경로가 코드화된 형태
- **skill-create / llm-dependent-agent-create (본인 글로벌)** — 본 노하우의 *제작* 경로가 코드화된 형태. 발굴이 막힌 지점의 답.
- **deploy-skill-기본 / deploy-subagent-기본 (본인 글로벌)** — 발굴·제작 결과를 편성·장착하는 메타 스킬
- **revfactory/skills** — 본인 1차 발굴 출처 저장소
- **Tool Atlas (본인 발굴 인프라, alpha)** — 본 챕터 결론의 다음 단계 자리. 참고자료 sources 참조.

## 참고자료

**「AI 에이전트 이해 및 제작방법」** — 에이전트의 권위 있는 정의와 5요소, 분류 체계(AI 플랫폼 종속형 / LLM 의존형), Claude Code 종속형 도구 6종, `skill-create` · `llm-dependent-agent-create` 제작 흐름, 멀티 에이전트까지 한 페이지로 정리한 종합 가이드. 본 챕터의 *제작* 경로 — *무엇을 만들 수 있고 어떻게 만드는가* — 를 깊이 다룬다. 찾기로 막혔을 때 제작으로 넘어가기 전에 한 번 통독할 자료.

🔗 [AI 에이전트 이해 및 제작방법 가이드](https://ai-agent-guide-finder-world.vercel.app/)
