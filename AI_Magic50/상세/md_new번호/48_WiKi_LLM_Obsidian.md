...
Andrej Karpathy가 제안한 **LLM 위키** 패턴(AI가 작업 기록을 마크다운 위키로 자동 정제·누적하는 방식)을 **Obsidian** 같은 마크다운 노트 도구 위에 얹어, 한 세션 안에서 끝나지 않는 **세션 너머의 장기 기억 인프라**를 만드는 노하우. AI 작업이 일회성으로 사라지지 않고, 과거 결정·실패·패턴이 다음 작업의 입력 자산으로 회수되게 만드는 운용 기반.
