...
대화형 모드는 본인이 키보드 앞에 있어야 한다. #9 1일 1영상을 진짜로 굴리려면 새벽 3시 본인이 자고 있어도 영상이 만들어져야 한다. `claude -p`는 그것을 가능하게 한다.

한 번 호출에 한 번 응답하고 종료한다. stdin/stdout이 표준 셸 파이프라인에 그대로 꽂힌다. 즉 Claude Code가 **`grep`·`jq`·`curl`과 같은 레벨의 CLI 도구**가 된다. #31(30개 운용), #14(소대 편제), #9(1일 1영상)이 진짜 자동화로 격상되는 분기점이다.
...
- `--model` : Opus/Sonnet/Haiku 명시 (#4와 짝)
- `--allowed-tools` : 자동화에서 허용할 도구만 화이트리스트
- `--dangerously-skip-permissions` : #13와 결합해 무인 실행
...
**주의 — UI 검증 철칙**: 헤드리스로 빌드·배포까지 자동화했다고 해서 "동작한다"고 단정하면 안 된다. **"curl 200 ≠ 동작함"**. 별도 Verification Agent가 Playwright로 클릭 테스트(#49)하지 않으면 SKILL_ATLAS 사고처럼 dead-link가 새벽에 양산된다.

## 관련 항목

- **#9 유튜브 1일 1영상** — 헤드리스 없이는 진짜 자동화 불가
- **#13 --dangerously-skip-permissions** — 무인 실행의 짝
- **#4 /model 명령** — 자동화에서도 모델 선택 의식
- **#49 스크린샷 자율 검증** — 헤드리스 결과의 UI 검증
