...
subtitle: "컨텍스트 사용량 / 5시간·7일 Rate Limit / 실행 중 명령 / Hook 발동 / 에이전트 투입현황 동시 표시"
type: C
group_id: 5
group_name: "환경"
order_in_group: 6
...
# 36. Claude Code Statusline HUD 설치해서 사용하기
...
직접 셸 스크립트로 statusline을 짜면 이 셋이 모두 해결되고, **외부 플러그인 의존 없는** 가시성 인프라가 완성된다.
...
**본인 운용 패턴**: 30개 인스턴스를 한 화면에 띄울 때 LINE2 막대그래프만 보면 어느 인스턴스를 우선 처리할지 즉시 판단된다. 분대원·분대장 호칭은 #14 군대 편제와 동일 어휘.

## 관련 항목

- **#14 군대 소대 편제** — 분대원/분대장 호칭 공유
- **#16 Subagent vs Agent Teams** — LINE4·LINE5가 두 메커니즘 별도 추적
- **#11 Hooks** — hook-logger.js가 LINE3B의 데이터 소스
- **#31 5컴퓨터 30개 운용** — 본 HUD가 그 가시성 인프라
